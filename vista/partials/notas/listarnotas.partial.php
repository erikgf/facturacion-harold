	...
	<div class="col-xs-6 col-sm-2">
		<div class="control-group">
			...
			<select  class="form-control" id="cbotipocomprobante-listar">
				<option value="07" selected>NOTA CRÉDITO</option>
	...
	<div class="col-xs-12">
		<div class="table-responsive">    		
			<table class="table tbl-detalle tablalista display text-nowrap" style="width:100%">
				...
				<script type="handlebars-x" id="tpl8ListaNotas">
					{{#data}}
						<tr >
							<td  style="width:100px" class="text-center">
								<button class="btn btn-xs btn-danger" title="Eliminar" onclick ="app.ListarNotas.anular({{id}})">
									<i class="fa fa-trash bigger-130"></i>
								</button>
								{{#../admin}}
								<!-- 
								<button class="btn btn-xs btn-warning" title="Editar" onclick ="app.RegistrarNotas.editar({{id}})">
									<i class="fa fa-edit bigger-130"></i>
								</button>
								-->
								{{/../admin}}
								<button class="btn btn-xs" title="Ver Detalle {{id}}" onclick ="app.ListarNotas.verDetalle({{id}})">
					...
					{{else}}
						<tr class="tr-null ">
							<td class="text-center" colspan="10">Sin Notas para mostrar</td>
						</tr>
					{{/data}}
				</script>
				</tbody>
